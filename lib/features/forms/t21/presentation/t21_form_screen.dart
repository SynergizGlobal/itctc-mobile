...
class T21FormScreen extends ConsumerStatefulWidget {
  const T21FormScreen({super.key, this.editIndex});
...
class _T21FormScreenState extends ConsumerState<T21FormScreen> {
  ...
  void _ensureLoaded() {
    ...
    if (widget.editIndex != null) {
      final rows = ref.read(t21TableProvider);
      if (widget.editIndex! < rows.length) {
        _entry = T21Entry.fromMap(rows[widget.editIndex!]);
        return;
      }
    }
    _entry = T21Entry();
  ...
  void _saveToTable() {
    ...
    if (widget.editIndex != null) {
      ref.read(t21TableProvider.notifier).updateRecord(widget.editIndex!, record);
    } else {
      ref.read(t21TableProvider.notifier).addRecord(record);
    }
    context.pop();
  }
